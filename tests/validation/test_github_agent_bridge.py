...
class GithubAgentBridgeTests(unittest.TestCase):
    def test_source_preflight_rejects_executable_config_before_worktree_inspection(self) -> None:
        for key in ("core.fsmonitor", "filter.new-name.clean", "filter.renamed.process",
                    "include.path", "includeif.gitdir:/novel/.path"):
            with self.subTest(key=key), tempfile.TemporaryDirectory() as directory:
                calls = []
                candidate = bridge(FakeTransport())
                def git(worktree, args, env=None):
                    calls.append(args)
                    if args == ["config", "--null", "--list", "--no-includes"]:
                        return key + "\nfixture-command\0"
                    self.fail("worktree inspection started before config rejection")
                candidate.git_runner = git
                with self.assertRaisesRegex(BridgeError, "publication preflight"):
                    candidate.push_feature_branch(Path(directory), "agent/issue-61-fixture")
    ...
    def test_every_public_text_field_uses_shared_submission_guards(self) -> None:
        samples = (
            "AK" + "IA" + "A" * 16,
            "gl" + "pat-" + "x" * 24,
            "xox" + "b-" + "x" * 24,
            ...
            "https://fixture:" + "x" * 16 + "@example.test",
            "fixture@" + "nvidia.com",
            "/" + "home/fixture/review.log",
            "work-" + "123456789" + ".brevlab.com",
        )
        for value in samples:
            for field in ("summary", "next", "blocker", "evidence"):
                changes = {field: value}
                if field == "evidence":
                    changes[field] = [{**event()["evidence"][0], "label": value}]
                with self.subTest(field=field, sample=value[:8]), self.assertRaises(BridgeError):
                    validate_event(event(**changes), REPOSITORY)
        with self.assertRaises(BridgeError):
            validate_event(event(branch="agent/" + samples[0]), REPOSITORY)

    def test_private_policy_extensions_apply_without_exposing_rejected_text(self) -> None:
        from scripts.validation.sensitive_content_audit import digest

        with tempfile.TemporaryDirectory() as directory:
            policy = Path(directory) / "private-policy.json"
            phrase = "fixture restricted publication detail"
            host = "fixture.private.example"
            policy.write_text(json.dumps({
                "schema": "sensitive-content-policy/1", "max_phrase_tokens": 6,
                "blocked_prefix_sha256": [], "blocked_phrase_sha256": [digest(phrase)],
                "blocked_host_sha256": [digest(host)],
            }))
            with patch.dict(os.environ, {"SENSITIVE_CONTENT_POLICY_FILE": str(policy)}):
                for value in (phrase, "runner." + host):
                    transport = FakeTransport()
                    with self.assertRaises(BridgeError) as raised:
                        bridge(transport).publish(event(summary=value), Path(directory) / "state.json")
                    self.assertNotIn(value, str(raised.exception))
                    self.assertEqual(transport.calls, [])
                policy.unlink()
                with self.assertRaisesRegex(BridgeError, "policy is unavailable") as raised:
                    validate_event(event(), REPOSITORY)
    ...
    def test_real_git_isolates_signatures_and_authenticated_push(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, {"AGENT_GITHUB_ALLOWED_SIGNERS_FILE": str(registry)}):
                run_git(source, ["config", "core.fsmonitor", str(trap)])
                with self.assertRaisesRegex(BridgeError, "publication preflight"):
                    candidate.push_feature_branch(source, branch)
                self.assertFalse(marker.exists())
                run_git(source, ["config", "--unset", "core.fsmonitor"])
