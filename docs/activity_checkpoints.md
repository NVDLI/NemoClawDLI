...
Remote progress and referrals default to off. The Activity panel explains the data use and offers an explicit connection choice under the published NVIDIA privacy notice. Referral tracking requires a separate choice. Choices persist within the tab only for the same delivered artifact and notice version; a changed build or notice requires enablement again. Disconnect cancels pending Activity operations and clears the remote session from this tab; local course work and checkpoint evidence remain available. It does not delete records already sent to NVIDIA.

Session identity comes from the delivered Pages or materialized manifest bytes and source version. Unvalidated source previews cannot enable collection. Policy schema 2 records a published notice and an operator enablement choice, without claiming a separate privacy or legal approval. NVIDIA's published policy supplies the general retention criteria and privacy-request route; it does not establish an Activity-specific deletion deadline or availability guarantee.

An authenticated lab may provide a same-origin transport through `lab_transport.base_path` in its deployed policy. Its `/artifact` route supplies the SHA-256 digest of the delivered materialized manifest, its source and adapter versions, and the lab's XSRF token. The lab must authenticate every request, enforce XSRF on writes, forward only supported Activity routes to the fixed HTTPS API, and exclude lab cookies and credentials from upstream requests. Set `allow_http` explicitly to support an HTTP lab; the panel warns learners before they choose to sync. HTTP permits network observers to read or alter the browser-to-lab session and progress. Public Pages continues to use HTTPS directly. Disabling collection remains an operator switch.
