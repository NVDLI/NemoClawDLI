...
AGENT_GITHUB_ALLOWED_SIGNERS_FILE=$HOME/.config/nemoclaw-agent/allowed-signers
```
...
Maintain the allowed-signers file outside the checkout, writable only by its owner. Each entry binds
the human author's email principal to their registered SSH signing public key. This file contains
public keys, not credentials. The bridge uses the system SSH verifier and a fresh bare repository;
checkout configuration, hooks and replacement refs cannot authorize a signature or receive the token.
...
Every commit beyond GitHub's current default-branch head must pass SSH signature, author-principal
and DCO verification before any push. Workflow edits are rejected across that complete range.
...
status moves to one pull-request comment. Repeated updates edit those records. The bridge verifies
the issue repository and current branch/PR head, then checks cached record scope before updating it.
Comment reuse requires the configured bot author and the same target. Lost state triggers remote
record discovery. A file lock serializes the read, sequence check, publication and state write.

This Check reports controller progress. A reported `pass` completes it with a `neutral` conclusion;
it cannot attest that validation passed. A reported failure retains a `failure` conclusion. Required
validation, review and release checks keep their own authority and must still pass independently.
Do not configure the lifecycle Check as a required validation gate.
...
blocker, and canonical evidence links to this public repository's issues, pull requests, commits or
runs. Query strings, fragments, encoded paths and alternate hosts are rejected. Keep these details
in the local dashboard only:
