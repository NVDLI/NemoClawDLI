...
try {
  ...
  if (pomeriumCall.headers.cookie !== '_pomerium=opaque.session') fail(`missing upstream Pomerium cookie: ${JSON.stringify(pomeriumCall.headers)}`);
  if (pomeriumCall.headers['x-pomerium-authorization']) fail('Pomerium request forwarded a caller-controlled authorization header');
