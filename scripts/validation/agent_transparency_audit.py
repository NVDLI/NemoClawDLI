...
REQUIRED_SOURCE = (
    ...
    '"AGENT_GITHUB_ALLOWED_SIGNERS_FILE"',
    'STATE_SCHEMA',
    'NoRedirect',
    'fcntl.LOCK_EX',
)
...
def is_publisher(text: str) -> bool:
    if not any(marker in text for marker in BRIDGE_MARKERS):
        return False
    try:
        tree = ast.parse(text)
    except SyntaxError:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Name) and isinstance(node.ctx, ast.Store) and node.id == "AGENT_TRANSPARENCY_BRIDGE":
            return True
        if isinstance(node, ast.Call):
            values = [*node.args, *(item.value for item in node.keywords)]
            if any(isinstance(value, ast.Constant) and value.value in ("POST", "PATCH", "PUT", "DELETE") for value in values):
                return True
    return False
...
def audit_texts(rows: dict[str, str]) -> list[str]:
    ...
    bridges = {
        ...
        if path.endswith(".py") and is_publisher(text)
    }
    if not bridges:
...
def self_test() -> list[str]:
    ...
    failures = []
    if is_publisher('request("GET", "/check-runs")'):
        failures.append("read-only check inspection was classified as a publisher")
    mutations = {
        ...
        "deleted-publisher": {path: text for path, text in base.items() if not path.endswith(".py")},
        "malformed-publisher": {**base, "scripts/ci/renamed_publisher.py": source + "\ndef malformed(:\n"},
    }
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        renamed = root / "scripts" / "ci" / "renamed_publisher.py"
        nested = root / "scripts" / "future" / "nested" / "operator.py"
        nested.parent.mkdir(parents=True)
        renamed.rename(nested)
        if audit(root):
            failures.append("nested renamed publisher did not pass discovery")
        nested.write_text(source.replace('"verify-commit"', '"not-a-verifier"'), encoding="utf-8")
        if not audit(root):
            failures.append("nested malformed authority contract escaped discovery")
        nested.unlink()
        if not audit(root):
            failures.append("publisher deletion escaped discovery")
    return failures
