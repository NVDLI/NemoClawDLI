...
CHECKS = [
    ...
    (
        ...
        "scripts/cors-proxy/cors-proxy-worker-openclaw.js",
        ["Neutral access sessions require an explicit access provider.", 'fwdHeaders.set("Cookie", "_pomerium=" + accessSession)', 'fwdHeaders.delete("CF-Access-Client-Id")', 'fwdHeaders.delete("CF-Access-Client-Secret")'],
    ...
    (
        ...
        [
            r"\bTRANSPORT\b",
            r"proxyControls\s*:\s*true",
            r"X-OpenClaw-Access-Session",
            r"CF-Access-Jwt-Assertion",
            r"helpers\.log\.json\(['\"]redacted response['\"]",
        ],
    ),
    (
        ...
        [
            ...
            "relayWebSocket === true",
            "Boolean(accessSession) && relayWebSocket !== false",
...
def self_test() -> list[str]:
    ...
    cases = (
        ...
        ("OpenClaw lesson four-route helper", "web/nemoclaw/03a-kickstart.html", "helpers.runOpenClawConnectionAudit({", "helpers.runSingleOpenClawCheck({"),
        ("OpenClaw lesson restores transport branch", "web/nemoclaw/03a-kickstart.html", "const connection = helpers.getOpenClawConnection();", "const TRANSPORT = 'direct';\nconst connection = helpers.getOpenClawConnection();"),
        ("OpenClaw lesson duplicates redacted output", "web/nemoclaw/03a-kickstart.html", "return redacted;", "helpers.log.json('redacted response', redacted);\nreturn redacted;"),
        ...
        ("OpenShell manual-session fallback", "web/nemoclaw/scripts/_openshell.js", "Boolean(accessSession) && relayWebSocket !== false", "Boolean(accessSession) && relayWebSocket === true"),
