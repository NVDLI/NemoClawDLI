...
CHECKS = [
    ...
    (
        "model reference worker owns the CORS answer it returns",
        "scripts/cors-proxy/cors-proxy-worker-build.js",
        ["deployable/src/proxy.mjs", "access-control-allow-credentials", "set-cookie",
         "for (const name of UPSTREAM_CORS_HEADERS) out.delete(name)",
         'out.set("Vary", "Origin")', 'fwd.delete("cookie")',
         'fwd.delete("x-dli-cors-proxy-secret")', "BILLING_INVOKE_ORIGIN.test(billingOrigin)",
         '["GET", "HEAD"].includes(request.method)'],
        [],
    ),
    (
        "launchable reference worker drops upstream credential grants and bounds downgraded redirects",
        "scripts/cors-proxy/cors-proxy-worker-openclaw.js",
        ["deployable/src/", "for (const name of UPSTREAM_CORS_HEADERS) out.headers.delete(name)",
         '"Access-Control-Allow-Credentials"', '"Set-Cookie2"',
         '"content-type", "content-length"]', '"Vary"', "host.split"],
        [],
    ),
    (
...
def self_test() -> list[str]:
    ...
    cases = (
        ...
        ("model worker replaces upstream CORS", "scripts/cors-proxy/cors-proxy-worker-build.js", "for (const name of UPSTREAM_CORS_HEADERS) out.delete(name);", "// upstream CORS headers kept"),
        ("model worker credential grant", "scripts/cors-proxy/cors-proxy-worker-build.js", '"access-control-allow-credentials",', "// credential grant kept"),
        ("model worker varies on origin", "scripts/cors-proxy/cors-proxy-worker-build.js", 'out.set("Vary", "Origin");', "// reflected origin cached for every caller"),
        ("model worker drops browser cookies", "scripts/cors-proxy/cors-proxy-worker-build.js", 'fwd.delete("cookie");', "// browser cookie forwarded upstream"),
        ("model worker drops edge secrets", "scripts/cors-proxy/cors-proxy-worker-build.js", 'fwd.delete("x-dli-cors-proxy-secret");', "// edge secret forwarded upstream"),
        ("model worker validates billing attribution", "scripts/cors-proxy/cors-proxy-worker-build.js", "BILLING_INVOKE_ORIGIN.test(billingOrigin)", "true"),
        ("model worker omits GET bodies", "scripts/cors-proxy/cors-proxy-worker-build.js", '["GET", "HEAD"].includes(request.method)', "false"),
        ("model worker names the complete implementation", "scripts/cors-proxy/cors-proxy-worker-build.js", "deployable/src/proxy.mjs", "an internal implementation"),
        ("launchable worker replaces upstream CORS", "scripts/cors-proxy/cors-proxy-worker-openclaw.js", "for (const name of UPSTREAM_CORS_HEADERS) out.headers.delete(name);", "// upstream CORS kept"),
        ("launchable worker credential grant", "scripts/cors-proxy/cors-proxy-worker-openclaw.js", '"Access-Control-Allow-Credentials",', "// upstream credential grant kept"),
        ("launchable worker upstream cookie", "scripts/cors-proxy/cors-proxy-worker-openclaw.js", '"Set-Cookie2",', "// upstream cookie kept"),
        ("launchable worker validates DNS labels", "scripts/cors-proxy/cors-proxy-worker-openclaw.js", 'host.split(".").every', "true"),
        ("launchable worker downgraded redirect body", "scripts/cors-proxy/cors-proxy-worker-openclaw.js", '"content-type", "content-length"]', '"content-type"]'),
        ("launchable worker names the complete implementation", "scripts/cors-proxy/cors-proxy-worker-openclaw.js", "deployable/src/", "an internal source tree"),
