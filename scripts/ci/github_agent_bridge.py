...
from scripts.validation import local_path_leak_audit, sensitive_content_audit
...
STATE_SCHEMA = "nemoclaw-agent-github-state/2"
...
def _public_line(value: object, field: str, limit: int, *, optional: bool = False) -> str | None:
    ...
    try:
        sensitive = sensitive_content_audit.scan_text(
            "submission", text, sensitive_content_audit.load_policy(),
        )
        local_paths = local_path_leak_audit.scan_text("submission", text)
    except Exception:
        raise BridgeError("public text policy is unavailable; publication is blocked") from None
    if sensitive or local_paths or any(pattern.search(text) for pattern in FORBIDDEN_PUBLIC_TEXT):
...
def validate_event(raw: object, expected_repository: str) -> dict[str, Any]:
    ...
    _public_line(branch, "branch", 127)
    head = raw["head_sha"]
    ...
    for index, item in enumerate(evidence):
        ...
        url = item["url"]
        route = r"(?:issues/[1-9][0-9]*|pull/[1-9][0-9]*|commit/[0-9a-f]{40}|runs/[1-9][0-9]*|actions/runs/[1-9][0-9]*)"
        if not isinstance(url, str) or not re.fullmatch(
            rf"https://github\.com/{re.escape(expected_repository)}/{route}", url,
        ):
            raise BridgeError(f"evidence[{index}].url must be a canonical public repository link without query or fragment")
...
class NoRedirect(HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        raise BridgeError("GitHub API redirects are not accepted")
...
def run_git(worktree: Path, arguments: list[str], env: dict[str, str] | None = None) -> str:
    # Repository-controlled hooks, signing programs and URL rewrites must never
    # receive publication credentials. Authenticated commands use a fresh bare repo.
...
class GithubBotBridge:
    ...
    def push_feature_branch(self, worktree: Path, expected_branch: str) -> dict[str, str]:
        ...
        if expected_branch in {"main", "master"} or expected_branch.startswith(("release/", "refs/", "tags/")):
            raise BridgeError("the agent bridge cannot publish a protected or release ref")
        configuration = self.git_runner(worktree, ["config", "--null", "--list", "--no-includes"], None)
        for entry in configuration.split("\0"):
            key = entry.split("\n", 1)[0].casefold()
            if key == "core.fsmonitor" or key.startswith(("filter.", "include.", "includeif.")):
                raise BridgeError("publication preflight requires a checkout without executable filters, monitors or config includes")
        ...
        self.verify()
        token = self.access_token()
        repository = self.transport("GET", f"/repos/{self.repository}", token, None)
        base_branch = repository.get("default_branch")
        if not isinstance(base_branch, str) or not BRANCH.fullmatch(base_branch) or expected_branch == base_branch:
            raise BridgeError("cannot publish the default branch")
        base_ref = self.transport("GET", f"/repos/{self.repository}/git/ref/heads/{base_branch}", token, None)
        base = base_ref.get("object", {}).get("sha", "")
        if not SHA.fullmatch(base):
            raise BridgeError("GitHub did not return the default branch head")
        with tempfile.TemporaryDirectory(prefix="agent-publication-") as directory:
            isolated = Path(directory)
            self.git_runner(isolated, ["init", "--bare"], None)
            self.git_runner(isolated, ["fetch", "--no-tags", str(worktree), head], None)
            if self.git_runner(worktree, ["rev-parse", "HEAD"], None) != head:
                raise BridgeError("feature branch changed during preflight")
            return self._publish_verified_range(isolated, worktree, base, head, expected_branch, token)

    def _publish_verified_range(
        self, worktree: Path, source: Path, base: str, head: str, expected_branch: str, token: str,
    ) -> dict[str, str]:
        self.git_runner(worktree, ["merge-base", "--is-ancestor", base, head], None)
        commits = self.git_runner(worktree, ["rev-list", "--reverse", f"{base}..{head}"], None).splitlines()
        if not commits:
            raise BridgeError("feature branch has no commits beyond the current default branch")
        signers = Path(os.environ.get("AGENT_GITHUB_ALLOWED_SIGNERS_FILE", "")).resolve()
        if signers == source or source in signers.parents or not signers.is_file():
            raise BridgeError("configure a trusted allowed-signers file outside the worktree")
        if signers.stat().st_mode & 0o022:
            raise BridgeError("allowed-signers file must not be writable by group or others")
        for commit in commits:
            if not SHA.fullmatch(commit):
                raise BridgeError("feature range contains an invalid commit")
            self._verify_human_commit(worktree, commit, signers)
        ...
        for commit in commits:
            remote_commit = self.transport("GET", f"/repos/{self.repository}/commits/{commit}", token, None)
            verification = remote_commit.get("commit", {}).get("verification", {})
            if remote_commit.get("sha") != commit or verification.get("verified") is not True:
                raise BridgeError("GitHub did not verify the published commit signature")
        return {"branch": expected_branch, "head_sha": head}

    def _verify_human_commit(self, worktree: Path, head: str, signers: Path) -> None:
        ...
        if "-----BEGIN SSH SIGNATURE-----" not in raw_commit:
            raise BridgeError("feature commits require an SSH signature from the trusted human signing registry")
        config = ["-c", "gpg.format=ssh", "-c", "gpg.ssh.program=/usr/bin/ssh-keygen",
                  "-c", f"gpg.ssh.allowedSignersFile={signers}"]
        self.git_runner(worktree, [*config, "verify-commit", head], None)
        principal = self.git_runner(worktree, [*config, "show", "-s", "--format=%GS", head], None)
        if principal != email:
            raise BridgeError("verified signer must match the human commit author email")
        changed = self.git_runner(worktree, ["diff-tree", "--no-commit-id", "--name-only", "-z", "-r", head], None)
        if any(path.startswith(".github/workflows/") for path in changed.split("\0")):
            raise BridgeError("the bot cannot publish workflow changes")

    def _verify_event_target(self, event: dict[str, Any], token: str) -> None:
        target = event["pull_request"] or event["issue"]
        issue = self.transport("GET", f"/repos/{self.repository}/issues/{target}", token, None)
        if issue.get("number") != target or issue.get("repository_url") != f"{API}/repos/{self.repository}":
            raise BridgeError("event issue does not belong to this repository")
        if event["head_sha"] is not None:
            ref = self.transport("GET", f"/repos/{self.repository}/git/ref/heads/{event['branch']}", token, None)
            if ref.get("object", {}).get("sha") != event["head_sha"]:
                raise BridgeError("event head is not the current branch head")
        if event["pull_request"] is not None:
            pull = self.transport("GET", f"/repos/{self.repository}/pulls/{target}", token, None)
            head = pull.get("head", {})
            if (head.get("ref") != event["branch"] or head.get("sha") != event["head_sha"]
                    or head.get("repo", {}).get("full_name") != self.repository):
                raise BridgeError("event does not match the pull request head")

    def _owns_comment(self, row: object, target: int) -> bool:
        return (isinstance(row, dict) and row.get("user", {}).get("login", "").casefold() == self.login.casefold()
                and row.get("issue_url") == f"{API}/repos/{self.repository}/issues/{target}"
                and str(row.get("body", "")).startswith(COMMENT_MARKER))
    ...
    def _upsert_comment(
        ...
        if comment_id is not None:
            row = self.transport("GET", f"/repos/{self.repository}/issues/comments/{comment_id}", token, None)
            if not self._owns_comment(row, target):
                raise BridgeError("cached comment is not the bot lifecycle record for this target")
        else:
            for page in range(1, 101):
                rows = self.transport(
                    "GET", f"/repos/{self.repository}/issues/{target}/comments?per_page=100&page={page}", token, None,
                )
                match = next((row.get("id") for row in rows if self._owns_comment(row, target)), None)
                comment_id = match if isinstance(match, int) else None
                if comment_id is not None or len(rows) < 100:
                    break
            else:
                raise BridgeError("comment discovery exceeded its bound; no new record was created")
    ...
    def _upsert_check(
        ...
        external_id = f"{self.repository}:{self.login}:{event['branch']}:{head}"
        if check_id is None:
            for page in range(1, 101):
                response = self.transport("GET", f"/repos/{self.repository}/commits/{head}/check-runs?filter=all&per_page=100&page={page}", token, None)
                rows = response.get("check_runs", [])
                match = next((row.get("id") for row in rows if row.get("name") == CHECK_NAME
                              and row.get("head_sha") == head and row.get("external_id") == external_id), None)
                check_id = match if isinstance(match, int) else None
                if check_id is not None or len(rows) < 100:
                    break
            else:
                raise BridgeError("check discovery exceeded its bound; no new record was created")
        if check_id is not None:
            row = self.transport("GET", f"/repos/{self.repository}/check-runs/{check_id}", token, None)
            if row.get("name") != CHECK_NAME or row.get("head_sha") != head or row.get("external_id") != external_id:
                raise BridgeError("cached check is not the lifecycle record for this exact head")
    ...
    def publish(self, raw_event: object, state_path: Path) -> dict[str, Any]:
        ...
        descriptor = os.open(str(state_path) + ".lock", os.O_CREAT | os.O_RDWR | getattr(os, "O_NOFOLLOW", 0), 0o600)
        with os.fdopen(descriptor, "w") as lock:
            fcntl.flock(lock, fcntl.LOCK_EX)
            return self._publish_locked(raw_event, state_path)

    def _publish_locked(self, raw_event: object, state_path: Path) -> dict[str, Any]:
        event = validate_event(raw_event, self.repository)
        state = read_state(state_path, self.repository, self.login)
        if event["sequence"] <= state.get("last_sequence", 0):
            raise BridgeError("event sequence must increase")
        body, output = render_event(event)
        self.verify()
        token = self.access_token()
        self._verify_event_target(event, token)
        state.setdefault("checks", {})
        cached_check = state["checks"].get(event["head_sha"])
        if cached_check is not None:
            row = self.transport("GET", f"/repos/{self.repository}/check-runs/{cached_check}", token, None)
            expected = f"{self.repository}:{self.login}:{event['branch']}:{event['head_sha']}"
            if row.get("name") != CHECK_NAME or row.get("head_sha") != event["head_sha"] or row.get("external_id") != expected:
                raise BridgeError("cached check is not the lifecycle record for this exact head")
...
def render_event(event: dict[str, Any]) -> tuple[str, dict[str, str]]:
    ...
    rows = [
        ...
        "",
        f"**Controller report: {event['phase']} · {event['state']}**",
        "",
    ...
    rows += [
        "",
        "_Lifecycle reports do not attest validation or authorize a merge or release. Consult the required checks and reviewed evidence._",
        "",
...
def read_state(path: Path, repository: str, login: str) -> dict[str, Any]:
    if not path.exists():
        return {}
    try:
    ...
    if state.get("schema") != STATE_SCHEMA or state.get("repository") != repository or state.get("login") != login:
        raise BridgeError("agent GitHub state scope or schema does not match")
    _positive_number(state.get("last_sequence"), "state.last_sequence")
    for field in ("comments", "checks"):
        values = state.get(field)
        if not isinstance(values, dict):
            raise BridgeError("agent GitHub state requires comment and check maps")
        for key, value in values.items():
            if not isinstance(key, str) or not re.fullmatch(r"[1-9][0-9]*" if field == "comments" else SHA, key):
                raise BridgeError("agent GitHub state contains an invalid target")
            _positive_number(value, f"state.{field}.id")
    return state
